...
class RelevanceLogger:
    ...
    def create_log_file(cls):
        ...
        log_files = [f for f in os.listdir(os.path.join("config", "logs", "relevance_log")) if f.startswith("rl_") and f.endswith(".log")]
        log_files.sort()
        if len(log_files) > max_log_files:
            oldest_file = log_files[0]
            oldest_file_path = os.path.join(log_file_path, oldest_file)
